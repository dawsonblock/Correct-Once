...
- `FastExecutor`
  - Handles `pure`
  - Handles `read`
  - Executes directly through Function Hooks gateway dispatch
  - Still runs lightweight policy hooks when the admission record requires them
...
private reads must stay explicit.

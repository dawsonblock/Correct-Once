#!/usr/bin/env python3
"""Verify native-kernel reproducibility against one independently recompiled second run.
...
def main() -> int:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("UPSTREAM_DAO_NATIVE_ENGINE_REPRODUCIBILITY.json"),
    )
    args = parser.parse_args()

    primary = (ROOT / args.primary).resolve() if not args.primary.is_absolute() else args.primary
    if not primary.is_file():
        raise SystemExit(f"primary native qualification is missing: {primary}")

    with tempfile.TemporaryDirectory(prefix="ef-dao-native-repro-") as temp_dir:
        second = Path(temp_dir) / "second.json"
        subprocess.run(
            [
                sys.executable,
                "scripts/qualify_upstream_dao_native_engine.py",
                "--donor-root",
                str(args.donor_root),
                "--output",
                str(second),
    ...
    document = {
        "schema": "effect-fabric/upstream-dao-native-engine-reproducibility/v1",
        "version": "0.2.11",

// Official durable-agent-outbox ConformanceHarness driving an Effect Fabric active compatibility
// engine. Unlike v0.2.4, this path never instantiates the donor OutboxWorker class. The scheduler
// implementation is compiled as EffectFabricActiveScheduler from the exact locked donor shell
// source, while every proposed CAS commit crosses a Python JSONL boundary into Effect Fabric's
// pre-commit invariant guard before reaching the Effect Fabric SQLite compatibility store.
//
// The donor pure reducer remains the protocol oracle in v0.2.5. This gate therefore qualifies an
// Effect Fabric-owned scheduling/persistence/enforcement path, not replacement of the donor reducer.
...
if (!donorRoot || !effectFabricRoot) {
  console.error("usage: node run_effect_fabric_active_engine.mjs <compiled-donor-root> <ef-root> [python]");
  process.exit(2);
}
...
const activePath = path.join(donorRoot, "packages", "core", "dist", "effectFabricActiveWorker.js");
const conformance = await import(pathToFileURL(conformancePath).href);
const core = await import(pathToFileURL(corePath).href);
const { EffectFabricActiveScheduler } = await import(pathToFileURL(activePath).href);
const {
  FakeTool,
  FAKE_RECEIPT_ISSUER,
  FAKE_RECEIPT_SECRET,
...
class PythonRpcStore {
  constructor() {
    this.tmp = fs.mkdtempSync(path.join(os.tmpdir(), "effect-fabric-dao-active-"));
...
class EffectFabricGuardedStore {
  constructor(base) {
    ...
    this.guard = new JsonRpcProcess(
      ...
      "Effect Fabric active DAO pre-commit guard",
    );
    this.guardChecks = 0;
  }
...
const report = await runConformance({
  implementation: "effect-fabric:active-scheduler-precommit-gate",
  createHarness: () => {
    ...
    const active = new EffectFabricActiveScheduler({
      store,
      tool: fake,
      receipts: fake,
...
const document = {
  schema: "effect-fabric/upstream-dao-active-engine-runner/v1",
  adapter: {
    outbox_engine_interface: "EffectFabricActiveScheduler",
    donor_outbox_worker_instantiated: false,
    transition_oracle: "locked @durable-agent-outbox/core pure reduce()",

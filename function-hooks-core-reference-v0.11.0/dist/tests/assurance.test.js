...
import { admitPinnedPlugins, computePluginDigest } from "@function-hooks/plugins";
...
test("hash-chain audit ledger detects tampering and redacts configured keys", async () => {
    ...
    assert.equal(entries[0].payload.input.token, "[REDACTED]");
    ...
    const verdict = ledger.verify(tampered);
...
//# sourceMappingURL=assurance.test.js.map
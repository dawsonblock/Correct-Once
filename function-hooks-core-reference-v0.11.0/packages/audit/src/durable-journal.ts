...
function parseJournal(raw: string, recoverTrailingPartial: boolean): { entries: AuditLedgerEntry[]; normalized: string; repaired: boolean } {
  if (!raw) return { entries: [], normalized: "", repaired: false };
  let normalized = raw;
  let repaired = false;
  if (!raw.endsWith("\n")) {
    if (!recoverTrailingPartial) throw new AuditRecoveryError("Audit journal ends with an unterminated record.");
  ...
  for (const [index, line] of normalized.split("\n").entries()) {
    ...
    try {
      entries.push(JSON.parse(line) as AuditLedgerEntry);
    } catch (error) {
...
export class DurableAuditJournal {
  ...
  readonly #options: DurableAuditJournalOptions;
  #ledger: HashChainAuditLedger;
  #appendQueue: Promise<void> = Promise.resolve();
  #persistenceFailure: Error | undefined;

  private constructor(path: string, ledger: HashChainAuditLedger, options: DurableAuditJournalOptions) {
    this.#path = path;
    this.#ledger = ledger;
    this.#options = options;
  }

  static async open(path: string, options: DurableAuditJournalOptions = {}): Promise<DurableAuditJournal> {
    ...
    return withFileLock(path, { enabled: options.crossProcessLock ?? true, ...(options.lockTimeoutMs === undefined ? {} : { timeoutMs: options.lockTimeoutMs }), ...(options.lockStaleMs === undefined ? {} : { staleMs: options.lockStaleMs }) }, async () => {
      let raw = "";
      try { raw = await readFile(path, "utf8") as string; }
      catch (error: any) {
      ...
      if (!check.ok) throw new AuditRecoveryError(`Audit chain verification failed at index ${check.index}: ${check.reason}`);
      if (parsed.repaired) await writeFile(path, parsed.normalized, "utf8");
      return new DurableAuditJournal(path, ledger, options);
    });
  }

  async #reloadFromDisk(): Promise<void> {
    const raw = await readFile(this.#path, "utf8") as string;
    ...
    const check = ledger.verify();
    if (!check.ok) throw new AuditRecoveryError(`Audit chain verification failed at index ${check.index}: ${check.reason}`);
    if (parsed.repaired) await writeFile(this.#path, parsed.normalized, "utf8");
    this.#ledger = ledger;
  }

  /** Refreshes this instance from durable state under the cross-process lock. */
  async refresh(): Promise<readonly AuditLedgerEntry[]> {
  ...
  entries(): readonly AuditLedgerEntry[] { return this.#ledger.entries(); }

  verify(): ReturnType<HashChainAuditLedger["verify"]> { return this.#ledger.verify(); }

  async append(payload: AuditPayload): Promise<AuditLedgerEntry> {
    let resolveEntry!: (entry: AuditLedgerEntry) => void;
    let rejectEntry!: (error: unknown) => void;
    const result = new Promise<AuditLedgerEntry>((resolve, reject) => { resolveEntry = resolve; rejectEntry = reject; });
    const task = this.#appendQueue.then(async () => {
      if (this.#persistenceFailure) throw new AuditRecoveryError("Audit journal is unavailable after a persistence failure; reopen and recover before continuing.", { cause: this.#persistenceFailure });
...
export async function verifyDurableAuditJournal(path: string, options: DurableAuditJournalOptions = {}): Promise<{ readonly ok: true; readonly entries: number }> {
  const journal = await DurableAuditJournal.open(path, { ...options, recoverTrailingPartial: false });
  const check = journal.verify();
  if (!check.ok) throw new AuditRecoveryError(`Audit verification failed at index ${check.index}: ${check.reason}`);

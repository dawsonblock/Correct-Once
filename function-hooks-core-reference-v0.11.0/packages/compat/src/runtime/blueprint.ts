...
export function emptyBlueprint(): EngineBlueprint {
  return Object.freeze({ events: new Map<string, EventDefinition>() });
}

export type EventAdditionInput = EventDefinition["invoke"] | EventAddition;

export function addEvents(
  below: EngineBlueprint,
  owner: string,
  additions: Readonly<Record<string, EventAdditionInput>>,
): EngineBlueprint {
  ...
  for (const [name, raw] of Object.entries(additions)) {
    ...
    const addition: EventAddition = typeof raw === "function" ? { invoke: raw } : raw;
    if (typeof addition.invoke !== "function") throw new InvalidBlueprintError(`Event ${name} has no invoke function.`);
...
export function filterEvents(below: EngineBlueprint, predicate: (definition: EventDefinition) => boolean): EngineBlueprint {
  return Object.freeze({ events: new Map([...below.events].filter(([, definition]) => predicate(definition))) });
}

export function validateBlueprintTransition(below: EngineBlueprint, above: EngineBlueprint): void {

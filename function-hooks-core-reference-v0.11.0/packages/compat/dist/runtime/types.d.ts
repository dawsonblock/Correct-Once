...
export type Hook<M extends EventMap, K extends EventName<M>> = (engine: EngineObject, event: DeepReadonly<EventInput<M, K>>, next: DispatchNext<M, K>) => MaybePromise<EventResult<M, K>>;
export type WildcardHook<M extends EventMap> = (engine: EngineObject, event: unknown, next: WildcardNext<M>) => MaybePromise<unknown>;
...
export type EngineCreateHook = (engine: Readonly<Record<string, never>>, event: EngineCreateEvent, next: EngineCreateNext) => MaybePromise<EngineBlueprint>;

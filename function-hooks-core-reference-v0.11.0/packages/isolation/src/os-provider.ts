...
import { ProcessIsolationLoader, type IsolationLaunchBuilder, type IsolationLaunchContext, type IsolationLaunchSpec, type NodePermissionProcessOptions } from "./process-loader.js";
...
function containerPluginPath(context: IsolationLaunchContext): string {
  const rel = relative(context.pluginRoot, context.modulePath).replaceAll("\\", "/");
  if (!rel || rel.startsWith("../") || rel === "..") throw new Error(`Plugin module ${context.modulePath} is outside plugin root ${context.pluginRoot}.`);
  return `/plugin/${rel}`;
}
 ...
 */
export function createPodmanLaunchBuilder(options: PodmanSandboxOptions = {}): IsolationLaunchBuilder {
  ...
  return (context: IsolationLaunchContext): IsolationLaunchSpec => {
...
export function summarizePodmanIsolationPlan(context: IsolationLaunchContext, options: PodmanSandboxOptions = {}): Readonly<Record<string, unknown>> {

...
export interface IsolationRuntimePort { dispatch(event: string, input: unknown, explicitOrigin?: string): Promise<unknown>; }
...
import { capabilityGranted, normalizeGrantSet, type CapabilityGrantResolver, type CapabilityGrantSet } from "./grants.js";
...
export function buildNodePermissionLaunchSpec(context: IsolationLaunchContext): IsolationLaunchSpec {
  return {
...
interface ActiveInvocation {
  readonly next: ((event: unknown) => Promise<unknown>) & { readonly signal: AbortSignal; readonly event: string; readonly origin: string };
}
...
class IsolatedProcessSession {
  readonly #runtime: IsolationRuntimePort;
  readonly #pluginName: string;
  ...
  #buffer = "";
  #readyResolve!: (hooks: readonly RemoteHookDescriptor[]) => void;
  #readyReject!: (error: Error) => void;
  readonly #ready: Promise<readonly RemoteHookDescriptor[]>;
  #closed = false;

  private constructor(runtime: IsolationRuntimePort, pluginName: string, child: ChildProcessWithoutNullStreams, grants: CapabilityGrantSet, options: Required<Pick<NodePermissionProcessOptions, "invocationTimeoutMs" | "maxProtocolLineBytes">>) {
    ...
    child.stderr.on("data", () => { /* plugin logs are intentionally not protocol */ });
    child.on("error", (error: Error) => this.#failAll(error));
    child.on("exit", (code: number | null, signal: string | null) => {
      if (!this.#closed) this.#failAll(new IsolationProtocolError(`Isolated plugin ${pluginName} exited unexpectedly (code=${String(code)}, signal=${String(signal)}).`));
    });
  }

  static async start(runtime: IsolationRuntimePort, modulePath: string, pluginName: string, pluginOptions: unknown, options: NodePermissionProcessOptions): Promise<IsolatedProcessSession> {
    ...
    const launchContext: IsolationLaunchContext = {
      pluginName,
  ...
  async attach(on: OnRegistrar): Promise<void> {
    const hooks = await this.#ready;
    for (const descriptor of hooks) {
      const callback = async (_engine: unknown, event: unknown, next: any): Promise<unknown> => this.#invoke(descriptor.hookId, event, next);
      if (descriptor.matcher === undefined) on(descriptor.event, callback);
      else on(descriptor.event, descriptor.matcher, callback);
    }
  }
  ...
  #send(message: HostToChildMessage): void {
    if (this.#closed || !this.#child.stdin.writable) throw new IsolationProtocolError(`Isolated plugin ${this.#pluginName} is not writable.`);
    const line = `${JSON.stringify(message)}\n`;
    if (Buffer.byteLength(line, "utf8") > this.#maxProtocolLineBytes) throw new IsolationProtocolError("Outbound isolation protocol message exceeds size limit.");
    this.#child.stdin.write(line);
  }

  async #invoke(hookId: string, event: unknown, next: ActiveInvocation["next"]): Promise<unknown> {
    const id = randomUUID();
    ...
    const onAbort = () => {
      try { this.#send({ type: "cancel", id }); } catch {}
    };
  ...
  #consume(chunk: string): void {
    ...
    while (true) {
      ...
      try { void this.#handle(JSON.parse(line) as ChildToHostMessage); }
      catch (error) {
  ...
  async #handle(message: ChildToHostMessage): Promise<void> {
    ...
    if (message.type === "invokeResult") {
      ...
      else pending.reject(new IsolationProtocolError(message.error));
      return;
    }
 ...
 */
export class ProcessIsolationLoader implements HooksModuleLoader {
  readonly #runtime: IsolationRuntimePort;
  readonly #options: NodePermissionProcessOptions;
  readonly #sessions: IsolatedProcessSession[] = [];
  ...
  async load(modulePath: string, context?: LoaderContext): Promise<HooksModule> {

...
function parentIsPlugin(parentURL: string | undefined): boolean {
  if (!parentURL || !pluginRoot || !parentURL.startsWith("file:")) return false;
  try {
...
export async function resolve(specifier: string, context: { parentURL?: string }, nextResolve: (specifier: string, context: unknown) => Promise<{ url: string }>): Promise<{ url: string }> {
  ...
  if (parentIsPlugin(context.parentURL)) {
    ...
    let real: string;
    try { real = realpathSync(path); } catch { throw new Error(`Isolated plugin import cannot be resolved safely: ${path}`); }
    if (!pluginRootReal || !within(real, pluginRootReal)) throw new Error(`Isolated plugin import escapes plugin root: ${real}`);
  }
  ...
  if (runtimeRoot && resolved.url.startsWith("file:")) {
    const path = resolvePath(fileURLToPath(resolved.url));
    if (within(path, runtimeRoot)) return resolved;
  }
  return resolved;
}

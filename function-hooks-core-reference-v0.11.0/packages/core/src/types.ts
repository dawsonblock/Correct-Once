...
export interface Next<M extends EventMap, K extends EventName<M>> {
  ...
  is<T extends EventName<M>>(
    type: T,
    event: unknown,
  ): event is DeepReadonly<EventInput<M, T>>;
}
...
export interface EngineEvent<M extends EventMap, K extends EventName<M>> {
  readonly invoke: (
    input: DeepReadonly<EventInput<M, K>>,
    engine: Engine<M>,
  ) => MaybePromise<EventResult<M, K>>;
}
...
export interface Runtime<M extends EventMap> {
  ...
  dispatch<K extends EventName<M>>(
    event: K,
...
export interface KernelBuilder<M extends EventMap> {
  ...
  on<K extends EventName<M>>(
    ...
    order: number,
    event: K,
    matcher: Matcher<EventInput<M, K>>,
    hook: Hook<M, K>,
  ): void;

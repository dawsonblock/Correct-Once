...
function baseBlueprint() {
  return createBlueprint<TestEvents>({
...
test("ordering wraps lower numeric order around higher order and preserves ties", async () => {
  const seen: string[] = [];
  const builder = createKernel<TestEvents>();
...
test("short circuit replaces the lower chain", async () => {
  let baseCalls = 0;
  const builder = createKernel<TestEvents>();
  builder.defineEngine(createBlueprint<TestEvents>({
...
test("event values reject exotic mutable containers instead of claiming false deep immutability", async () => {
  const builder = createKernel<any>();
  builder.defineEngine(createBlueprint<any>({
    "data.echo": { invoke: (input: unknown) => input },
  }));
  const runtime = builder.build();
  await runtime.start();
...
test("recursive nested dispatch skips only the currently executing hook", async () => {
  ...
  builder.on("recursive", 0, "math.add", async (engine, event, next) => {
    ...
    if (event.a === 1) {
      const nested = await engine.math!.add!({ a: 2, b: 3 }) as number;
...
test("engine exposes only blueprint events and unknown dispatch fails", async () => {
  ...
  await assert.rejects(() => (runtime as any).dispatch("fs.read", {}), UnknownEventError);
});
...
test("hook and base failures are distinguishable and preserve cause", async () => {
  const baseBuilder = createKernel<TestEvents>();
  baseBuilder.defineEngine(createBlueprint<TestEvents>({
  ...
  await assert.rejects(async () => {
    try { await baseRuntime.dispatch("math.add", { a: 1, b: 2 }); }
    catch (error) {
      assert.ok(error instanceof BaseExecutionError);
      assert.ok((error as BaseExecutionError).cause instanceof TypeError);
      throw error;
    }
  }, BaseExecutionError);

  const passBuilder = createKernel<TestEvents>();
  passBuilder.defineEngine(createBlueprint<TestEvents>({
  ...
  const hookBuilder = createKernel<TestEvents>();
  ...
  await assert.rejects(async () => {
    try { await hookRuntime.dispatch("math.add", { a: 1, b: 2 }); }
    catch (error) {
      assert.ok(error instanceof HookExecutionError);
      assert.ok((error as HookExecutionError).cause instanceof RangeError);
      throw error;
    }
  }, HookExecutionError);
});
...
test("caller cancellation and deadline have distinct errors", async () => {
  const builder = createKernel<TestEvents>();
  builder.defineEngine(createBlueprint<TestEvents>({
    "math.add": { invoke: async () => new Promise<number>(() => {}) },
    "log.write": { invoke: () => undefined },
  }));
  const runtime = builder.build();
  await runtime.start();
...
test("golden three-hook trace has deterministic phase order", async () => {
  ...
  const builder = createKernel<TestEvents>({ trace: (row) => trace.push(row) });
  builder.defineEngine(baseBlueprint());
...
test("trace observer failures are isolated from dispatch semantics", async () => {
  let calls = 0;
  const builder = createKernel<TestEvents>({ trace: () => { calls += 1; throw new Error("observer failed"); } });
  builder.defineEngine(baseBlueprint());

...
export interface FileReadInput extends GatewayActionBase { readonly path: string; }
export interface FileWriteInput extends GatewayActionBase { readonly path: string; readonly data: string; }
...
export interface FileReadResult { readonly data: string; readonly bytes: number; }
export interface FileWriteResult { readonly bytesWritten: number; }
export interface ProcessExecResult { readonly exitCode: number; readonly stdout: string; readonly stderr: string; }
...
export type GatewayEvents = {
  "mcp.call": { input: McpCallInput; result: unknown };
  "browser.call": { input: BrowserCallInput; result: unknown };
  "desktop.call": { input: DesktopCallInput; result: unknown };
  "fs.read": { input: FileReadInput; result: FileReadResult };
  "fs.write": { input: FileWriteInput; result: FileWriteResult };
  "process.exec": { input: ProcessExecInput; result: ProcessExecResult };
  "network.request": { input: NetworkRequestInput; result: NetworkRequestResult };
};
...
export type GatewayAdapter<K extends GatewayEventName> = (
  input: DeepReadonly<GatewayEvents[K]["input"]>,
  context: GatewayAdapterContext,
) => MaybePromise<GatewayEvents[K]["result"]>;

export type GatewayAdapters = Partial<{ [K in GatewayEventName]: GatewayAdapter<K> }>;
...
export interface AgentGateway {
  ...
  readonly engine: Engine<GatewayEvents>;
  dispatch<K extends GatewayEventName>(event: K, input: GatewayEvents[K]["input"], options?: { readonly origin?: string; readonly signal?: AbortSignal; readonly timeoutMs?: number }): Promise<GatewayEvents[K]["result"]>;
  close(): Promise<void>;
}

export interface GatewayTraceOptions { readonly trace?: (record: RuntimeTraceRecord) => void; readonly defaultTimeoutMs?: number; }

export const GATEWAY_EVENTS = Object.freeze([
  "mcp.call", "browser.call", "desktop.call", "fs.read", "fs.write", "process.exec", "network.request",
] as const satisfies readonly GatewayEventName[]);

export const DEFAULT_RECEIPT_EVENTS = Object.freeze([
  "mcp.call", "browser.call", "desktop.call", "fs.write", "process.exec", "network.request",
] as const satisfies readonly GatewayEventName[]);

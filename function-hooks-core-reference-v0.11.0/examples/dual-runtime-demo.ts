...
import { createKernel, type RuntimeTraceRecord } from "@function-hooks/core";
...
async function exercise(label: string, factory: ConformanceKernelFactory) {
  const trace: RuntimeTraceRecord[] = [];
  const builder = factory<DemoEvents>({ trace: (row) => trace.push(row) });
  builder.defineEngine({
    events: new Map([
      ["math.add", { invoke: ({ a, b }: { readonly a: number; readonly b: number }) => a + b }],
    ]),
  });

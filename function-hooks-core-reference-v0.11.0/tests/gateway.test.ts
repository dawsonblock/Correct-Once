...
import {
  GatewayApprovalDeniedError,
  GatewayDeniedError,
  GatewayHostPolicyError,
  GatewayInvalidRewriteError,
  GatewayOutputLimitError,
  allowAllGatewayAuthorizer,
  createAgentGateway,
  createGatewayAllowlistAuthorizer,
  createNodeGatewayAdapters,
  gatewayHashChainAuditSink,
  type GatewayAuditRecord,
} from "@function-hooks/gateway";
import { DispatchCancelledError } from "@function-hooks/core";

const mcpInput = (actionId: string, tool = "safe") => ({ actionId, server: "local", tool, args: { x: 1 } });
...
test("policy denial and approval denial are audited before execution", async () => {
  const records: GatewayAuditRecord[] = []; let called = 0;
  ...
  assert.equal(called, 0); assert.equal(records.at(-1)?.phase, "policy.denied");
  await denied.close();
  ...
  assert.equal(called, 0); assert.equal(records.at(-1)?.phase, "approval.denied");
...
test("authorized rewrite is revalidated and the executor sees the effective action", async () => {
  ...
  const gateway = await createAgentGateway({
    ...
    authorizer: (context) => ({ effect: "allow", rewrite: { ...(context.input as any), tool: "rewritten-safe" } }),
  });
  const result = await gateway.dispatch("mcp.call", mcpInput("rewrite", "unsafe"));
  assert.equal(seen, "rewritten-safe"); assert.deepEqual(result, { tool: "rewritten-safe" });
  await gateway.close();
});
...
test("policy rewrites cannot replace the immutable actionId", async () => {
  ...
  const gateway = await createAgentGateway({
    adapters: { "mcp.call": async () => { called += 1; return null; } }, receipts: false,
    authorizer: (context) => ({ effect: "allow", rewrite: { ...(context.input as any), actionId: "replacement-id" } }),
  });
...
test("receipts replay completed actions and reject action-id conflicts", async () => {
  let calls = 0; const store = new InMemoryActionReceiptStore(); const records: GatewayAuditRecord[] = [];
  const gateway = await createAgentGateway({
...
test("caller cancellation reaches the execution adapter but does not claim rollback", async () => {
  let started = false; let observedAbort = false;
  const gateway = await createAgentGateway({
  ...
  assert.equal(started, true); assert.equal(observedAbort, true);
  await gateway.close();
});
...
test("hash-chain gateway audit records the effective execution path", async () => {
  ...
  assert.deepEqual(ledger.entries().map((entry) => (entry.payload.input as any).phase), ["execution.start", "execution.success"]);
  await gateway.close();
});


test("allowlist policy constrains MCP tools and browser operations before host callbacks", async () => {
  let mcpCalls = 0; let browserCalls = 0;
  const gateway = await createAgentGateway({
    receipts: false,
  ...
  assert.equal(mcpCalls, 1); assert.equal(browserCalls, 1);
  await gateway.close();
});

test("failure after an external side effect leaves the receipt indeterminate and blocks automatic retry", async () => {
  const store = new InMemoryActionReceiptStore(); let calls = 0;
  const gateway = await createAgentGateway({
    authorizer: allowAllGatewayAuthorizer(), receipts: { store },
    adapters: { "mcp.call": async () => ({ call: ++calls }) },
    audit: (record) => { if (record.phase === "execution.success") throw new Error("audit persistence failed after side effect"); },
  });
  await assert.rejects(gateway.dispatch("mcp.call", mcpInput("indeterminate")));
  assert.equal(calls, 1); assert.equal((await store.get("indeterminate"))?.state, "indeterminate");
...
test("Node process adapter uses capability profiles, rejects arbitrary args, and never inherits host environment", async () => {
  ...
  try {
    ...
    assert.equal(result.exitCode, 0); assert.equal(result.stdout, "allowed|hidden");
    ...
    await gateway.close();
  } finally { delete process.env.FH_TEST_SECRET; }
});
...
test("Node network adapter allows exact origins, blocks redirects by default and rejects other origins", async () => {
  ...
  const server = createServer((req: any, res: any) => {
    ...
    if (req.url === "/large") { res.statusCode = 200; res.end("x".repeat(128)); return; }
    res.statusCode = 200; res.setHeader("content-type", "text/plain"); res.end("local-ok");
  });
  ...
  try {
    ...
    assert.equal(response.status, 200); assert.equal(response.body, "local-ok");

...
test("receipt results are immutable snapshots and caller mutation cannot corrupt replay", async () => {
  ...
  const first = await gateway.dispatch("mcp.call", { actionId: "snapshot", server: "s", tool: "t" }) as any;
  first.nested.x = 999;
  const replay = await gateway.dispatch("mcp.call", { actionId: "snapshot", server: "s", tool: "t" }) as any;
  assert.equal(calls, 1);
...
test("file receipt recovery verifies persisted result hashes and transition order", async () => {
  ...
  const lines = (await readFile(path, "utf8") as string).trimEnd().split("\n");
...
test("approved policy rewrites are frozen before approval and execution", async () => {
  ...
  const gateway = await createAgentGateway({
    receipts: false,
    authorizer: (context) => ({ effect: "allow", approval: "required", rewrite: { ...(context.input as any), tool: "safe" } }),
    approval: (context) => {
      assert.equal(Object.isFrozen(context.input), true);
      assert.throws(() => { (context.input as any).tool = "mutated-after-validation"; });
      return true;
    },
...
test("timed-out processes that ignore SIGTERM are escalated and cannot keep running", async () => {
  ...
  let survived = true;
  try { await readFile(marker, "utf8"); } catch { survived = false; }

...
async function childNode(source: string, args: readonly string[]): Promise<string> {
...
test("process-group termination kills descendants after timeout", async () => {
  ...
  try {
    ...
    await delay(1200);
    await assert.rejects(readFile(marker, "utf8"), (error: any) => error?.code === "ENOENT");
    await gateway.close();
  } finally { await rm(root, { recursive: true, force: true }); }
});
...
test("network adapter rejects allowlisted private destinations by default and permits explicit private test mode", async () => {
  ...
  try {
    ...
    assert.equal(result.status, 200); assert.equal(result.body, "ok");
    await allowed.close();

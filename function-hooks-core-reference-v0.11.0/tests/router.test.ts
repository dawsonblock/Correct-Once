...
test("one execution router maps many applications onto reusable gateway backends", async () => {
  ...
  assert.equal((await router.execute({ actionId: "slack-1", app: "slack", capability: "message.post", input: { text: "hello" } }) as any).body, "sent");
  assert.equal((await router.execute({ actionId: "web-1", app: "amazon", capability: "search", input: { q: "keyboard" } }) as any).operation, "navigate");
  assert.equal((await router.execute({ actionId: "desk-1", app: "vscode", capability: "file.open", input: { path: "src/index.ts" } }) as any).application, "vscode");
  assert.equal((await router.execute({ actionId: "local-1", app: "code", capability: "test" }) as any).stdout, "tests-pass");
...
test("route builders cannot forge the gateway action identity", async () => {
  ...
  } as unknown as AnyCapabilityRoute;
...
test("route backend labels must match the gateway event they describe", async () => {
  const gateway = await createAgentGateway({ receipts: false, authorizer: allowAllGatewayAuthorizer(), adapters: {} });
  const invalid = { app: "bad", capability: "mapping", backend: "api", event: "mcp.call", build: () => ({ server: "s", tool: "t" }) } as unknown as AnyCapabilityRoute;

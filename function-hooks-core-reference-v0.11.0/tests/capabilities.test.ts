...
import {
  compileAdmittedCapabilityRoute,
  createExecutionRouter,
  createExecutionRouterFromRegistry,
  mcpCapabilityRoute,
} from "@function-hooks/router";

function candidate(overrides: Partial<Parameters<typeof createCapabilityCandidate>[0]> = {}) {
...
function admitted(overrides: Partial<Parameters<typeof createCapabilityCandidate>[0]> = {}) {
...
test("admission binds the exact candidate and raw candidates are not routable", async () => {
  ...
  assert.throws(() => compileAdmittedCapabilityRoute(raw as unknown as AdmittedCapability), CapabilityAdmissionError);

  const tampered = { ...good, candidateHash: "0".repeat(64) } as AdmittedCapability;
...
test("registry accepts admitted capabilities only and has irreversible terminal states", async () => {
  ...
  const cap = admitted();
  await assert.rejects(registry.register(candidate() as unknown as AdmittedCapability), CapabilityAdmissionError);
...
test("agent catalog is projected from active registry records and omits backend implementation details", async () => {
  ...
  assert.equal("implementation" in (catalog[0] as object), false);
  assert.equal("provenance" in (catalog[0] as object), false);
  assert.equal("admission" in (catalog[0] as object), false);
});
...
test("router can be compiled automatically from active admitted capabilities", async () => {
  ...
  assert.equal((await router.execute({ actionId: "b", app: "dev", capability: "tests.run", input: ["--runInBand"] }) as any).stdout, "project-tests");
  assert.equal((await router.execute({ actionId: "c", app: "workspace", capability: "file.read", input: "src/index.ts" }) as any).data, "src/index.ts");
